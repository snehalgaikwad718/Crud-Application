...
if(isset($_POST["Submit"])) {
  if(!empty($_POST["name"])&&!empty($_POST["phone"])) {
    ...
    $Salary = $_POST["salary"];
    $ConnectingDB;
    $sql = "UPDATE emp_details SET ename='$EName', phone='$Phone', 
    homeaddress='$Address', street='$Street', city='$City', ssn='$Ssn', 
    dept='$Dept', salary='$Salary' WHERE id='$SearchQueryParameter'";
    $Execute = $ConnectingDB->query($sql);
    if($Execute) {
      echo '<script>window.open("View_into_database.php?id=Record Updated Successfully", "_self")</script>';
    }
  }
}
...
<html>
  ...
  <body>
    ...
    <form action="Update.php?id=<?php echo $SearchQueryParameter; ?>" method="POST">
      ...
      <button type="submit" name="Submit" href="/">Update Record</button>
    </form>
    </div> 
  </body>
</html>